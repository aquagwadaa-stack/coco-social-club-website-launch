...
import { ArrowRight, CheckCircle2, Clock, MessageCircle, Star, Target, Users } from "lucide-react";
import { z } from "zod";
  ...
  head: () => ({
    meta: [
      { title: "Séance EMS découverte gratuite — Coco Social Club Le Moule" },
      {
        name: "description",
        content:
          "Découvre l'électrostimulation au Coco Social Club au Moule. 20 minutes coachées, gratuit. Réservation en 1 minute.",
      },
      { property: "og:title", content: "Séance EMS gratuite — Coco Social Club" },
      {
        property: "og:description",
        content: "20 minutes avec un coach dédié. Séance découverte offerte.",
      },
...
function Landing() {
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setErrors({});
    const message = [
      "Bonjour, je souhaite réserver une séance découverte EMS au Coco Social Club.",
      `Prénom : ${r.data.prenom}`,
      `Téléphone : ${r.data.phone}`,
      `Email : ${r.data.email}`,
    ].join("\n");
    window.open(
      `https://wa.me/590690979545?text=${encodeURIComponent(message)}`,
      "_blank",
      "noopener,noreferrer",
    );
    setSent(true);
  ...
  return (
    <div className="bg-background min-h-screen">
        ...
        <div className="container-editorial grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="mt-6 text-lg text-muted-foreground max-w-lg">
              Une séance personnalisée de 20 minutes avec un coach dédié.{" "}
              <strong className="text-foreground">Offerte.</strong>
            </p>
            ...
            <ul className="mt-8 grid sm:grid-cols-2 gap-3">
              {[
                ...
                <li
                  key={b.t}
                  className="flex items-center gap-2 text-sm rounded-full bg-cream border border-border/60 px-4 py-2"
                >
                  <span className="text-taupe">{b.icon}</span>
                  {b.t}
                </li>
              ))}
            </ul>
            ...
            <div className="mt-8 hidden lg:block">
              <a
                href="#form"
                className="btn-pill bg-primary text-primary-foreground text-base py-4 px-6"
              >
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-6 -left-6 rounded-[1.5rem] bg-primary text-primary-foreground p-5 shadow-xl max-w-xs hidden md:block">
              <p className="font-serif text-3xl leading-tight">
                20 min = 4 h de sport traditionnel.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="container-editorial grid md:grid-cols-2 gap-10 items-center max-w-4xl">
          <div>
            ...
            <p className="mt-4 text-muted-foreground">
              Un coach te rappelle pour fixer le créneau qui te convient.
            </p>
          </div>
          <div>
            {sent ? (
              <div className="rounded-[1.5rem] border border-border bg-background p-8 text-center fade-up">
                ...
                <p className="mt-2 text-muted-foreground text-sm">
                  Ta demande est prête dans WhatsApp. Envoie-la pour être rappelé(e) et choisir ton
                  créneau.
                </p>
              </div>
            ) : (
              <form
                onSubmit={submit}
                noValidate
                className="rounded-[1.5rem] border border-border bg-background p-6 grid gap-3"
              >
                ...
                <button className="btn-pill bg-primary text-primary-foreground mt-2">
                  Continuer sur WhatsApp <MessageCircle className="h-4 w-4" />
                </button>
              </form>
            )}
          </div>
      ...
      <section className="container-editorial py-20 text-center">
        <p className="font-serif italic text-2xl max-w-xl mx-auto">
          « Testez. Approuvez. Adoptez. »
        </p>
        <Link to="/reservation" className="btn-pill mt-8 border border-border">
  ...
  return (
    <div>
      ...
      <input
        ...
        name={name}
        type={type}
        className="mt-1 w-full rounded-full border border-border bg-cream/40 px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
      />
