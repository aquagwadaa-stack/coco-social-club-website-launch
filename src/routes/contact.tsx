...
import { CheckCircle2, Clock, Instagram, Mail, MapPin, MessageCircle, Phone } from "lucide-react";
import { z } from "zod";
  ...
  head: () => ({
    meta: [
      { title: "Contact — Coco Social Club, 6 rue de Portland, Le Moule" },
      {
        name: "description",
        content:
          "Contacte Coco Social Club au Moule : WhatsApp, téléphone, formulaire. 6 rue de Portland, 97160 Le Moule, Guadeloupe.",
      },
...
function Contact() {
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setErrors({});
    const message = [
      "Bonjour, je vous contacte depuis le site Coco Social Club.",
      `Nom : ${result.data.name}`,
      `Email : ${result.data.email}`,
      result.data.phone ? `Téléphone : ${result.data.phone}` : "",
  ...
  return (
    <SiteLayout>
      <PageHero
        eyebrow="Contact"
        title="Un mot, un appel, un message."
        subtitle="On se fait un plaisir de te répondre rapidement — en français ou en créole."
      />
      ...
      <section className="container-editorial pb-24 grid lg:grid-cols-12 gap-10">
        <div className="lg:col-span-5 space-y-6">
          ...
          <InfoBlock icon={<Phone className="h-4 w-4" />} title="Téléphone">
            <a href="tel:+590690979545" className="underline underline-offset-4">
              +590 690 97 95 45
            </a>
            <p className="text-xs text-muted-foreground mt-1">
              Cryolipolyse : <a href="tel:0690970646">06 90 970 646</a>
            </p>
            <p className="text-xs text-muted-foreground">
              Pole Dance : <a href="tel:0699667325">06 99 66 73 25</a>
            </p>
          </InfoBlock>
          <InfoBlock icon={<Clock className="h-4 w-4" />} title="Horaires">
            Lun. — Ven. selon planning
            <br />
            Cours en soirée · soins sur rendez-vous
          </InfoBlock>
          <InfoBlock icon={<Mail className="h-4 w-4" />} title="Email">
            <a href="mailto:cocosocialclub1@gmail.com" className="underline underline-offset-4">
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="btn-pill border border-border"
            >
          ...
          {sent ? (
            <div className="rounded-[1.5rem] border border-border bg-cream p-10 text-center fade-up">
              ...
              <p className="mt-2 text-muted-foreground">
                Ta demande est prête dans WhatsApp. Il ne te reste plus qu’à l’envoyer.
              </p>
            </div>
          ) : (
            <form
              onSubmit={submit}
              noValidate
              className="rounded-[1.5rem] border border-border bg-background p-6 md:p-8 grid gap-4"
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  name="message"
                  rows={5}
                  className="mt-2 w-full rounded-2xl border border-border bg-cream/40 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                />
                {errors.message && (
              ...
              <p className="text-xs text-muted-foreground">
                En envoyant ce formulaire, tu acceptes d'être recontacté(e) par l'équipe Coco.
              </p>
            </form>
          )}
        </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        name={name}
        type={type}
        className="mt-2 w-full rounded-full border border-border bg-cream/40 px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
      />
