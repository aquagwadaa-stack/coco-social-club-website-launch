...
const PRESTATIONS: Record<
  Univers,
  { id: string; name: string; duration: string; note?: string }[]
> = {
  sport: [
    {
      id: "ems",
      name: "Séance EMS (20 min)",
      duration: "20 min",
      note: "1re séance découverte gratuite",
    },
  ...
  "bien-etre": [
    {
      id: "cryo",
      name: "Cryolipolyse — jusqu'à 3 zones",
      duration: "60 min",
      note: "Bilan préalable inclus",
    },
...
export const Route = createFileRoute("/reservation")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Réserve ta séance sport ou bien-être au Coco Social Club en quelques étapes.",
      },
...
function Reservation() {
  ...
  const whatsappMessage = [
    "Bonjour, je souhaite demander une réservation au Coco Social Club.",
    `Prestation : ${prestationName || ""}`,
    `Date souhaitée : ${date || ""}`,
    `Créneau souhaité : ${slot || ""}`,
    `Nom : ${coords.prenom} ${coords.nom}`,
    `Téléphone : ${coords.phone}`,
    `Email : ${coords.email}`,
  ].join("\n");
  const whatsappUrl = `https://wa.me/590690979545?text=${encodeURIComponent(whatsappMessage)}`;
                ...
                {(["sport", "bien-etre"] as Univers[]).map((u) => (
                  ...
                  >
                    ...
                    <p className="mt-3 font-serif text-3xl">
                      {u === "sport" ? "Sport" : "Bien-être"}
                    </p>
                    <p
                      className={
                        "mt-3 text-sm " + (univers === u ? "opacity-90" : "text-muted-foreground")
                      }
                    >
                      {u === "sport"
                        ? "EMS, coaching, cours collectifs."
                        : "Cryolipolyse, pressothérapie, tan, massages."}
                    </p>
                  </button>
                ))}
              </div>
                    ...
                    <div>
                      ...
                      {p.note && (
                        <p
                          className={
                            "text-xs mt-1 " +
                            (prestation === p.id ? "opacity-80" : "text-muted-foreground")
                          }
                        >
                          {p.note}
                        </p>
              ...
              <div className="mt-8 grid gap-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <Input
                    ...
                    label="Prénom"
                    value={coords.prenom}
                    onChange={(v) => setCoords({ ...coords, prenom: v })}
                    error={errors.prenom}
                  />
                  <Input
                    name="nom"
                    label="Nom"
                    value={coords.nom}
                    onChange={(v) => setCoords({ ...coords, nom: v })}
                    error={errors.nom}
                  />
                </div>
                <Input
                  ...
                  type="tel"
                  value={coords.phone}
                  onChange={(v) => setCoords({ ...coords, phone: v })}
                  error={errors.phone}
                />
                <Input
                  name="email"
                  ...
                  type="email"
                  value={coords.email}
                  onChange={(v) => setCoords({ ...coords, email: v })}
                  error={errors.email}
                />
                <button
                  onClick={submitCoords}
                  className="btn-pill bg-primary text-primary-foreground mt-2 self-start"
                >
          ...
          {step === 6 && (
            <>
              ...
              <p className="mt-5 text-sm text-muted-foreground">
                L’équipe confirme ensuite la disponibilité du créneau. Aucun paiement n’est demandé
                à cette étape.
              </p>
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                >
                  Envoyer ma demande sur WhatsApp <MessageCircle className="h-4 w-4" />
                </a>
                <a
                  href="https://member-app.deciplus.pro/cocosc/shop"
                  target="_blank"
                  rel="noreferrer"
